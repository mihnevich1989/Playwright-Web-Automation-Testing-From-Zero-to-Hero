...
test('web tables', async ({ page }) => {
  ...
  await page.locator('input-editor').getByPlaceholder('E-mail').fill('text@test.com')
  await page.locator('.nb-checkmark').click()

  await expect(targetRowById.locator('td').nth(5)).toHaveText('text@test.com')
...
test('sliders', async ({ page }) => {
  ...
  await tempGauge.evaluate(node => {
    node.setAttribute('cx', '228')
    node.setAttribute('cy', '228' )
  })
